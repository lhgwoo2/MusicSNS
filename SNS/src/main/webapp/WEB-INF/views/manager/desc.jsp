...
<script type="text/javascript">
	...
	function delVideo(num) {
		var serialData = {};
		serialData.num = num;
		alert(serialData.num);
		$.ajax({
			url : '/SNS/video/del',
			data : serialData,
			type : 'get',
	...
	function delAudio(num) {
		...
		$.ajax({
			...
			success : function(res) {
				if (res.ok) {
					alert('삭제성공');
					location.href = "/SNS/recodes/getList";
				} else {
					alert('삭제실패');
				}
			},
	...
	function delPromotion(num) {
		var serialData = {};
		serialData.num = num;
		alert(serialData.num);
		$.ajax({
			url : '/SNS/promotion/delete',
			data : serialData,
			type : 'post',
			dataType : 'json',
			success : function(res) {
				if (res.ok) {
					alert('삭제성공');
					location.href = "/SNS/promotion/list";
				} else {
					alert('삭제실패');
				}
			},
	...
	<!--navbar 오른쪽 부분 -->
			<div class="collapse navbar-collapse" id="example-navbar-primary">
				<ul class="nav navbar-nav navbar-left">
			        ...
			        <li class="active"><a href="<c:url value='/manager/list'/>">Manager</a></li>
			        </sec:authorize>
				</ul>
  ...
  <div class="row content">
    ...
    <br>
     ...
     <div class="main main-raised">
      ...
      <h1>신고 게시물 상세 정보</h1>
      <p>유저가 신고한 게시물 상세 정보</p>
      </span>
      <hr>
      <p></p>
      
     <div class="container text-center" style="min-width:900px;" >
      <sec:authentication property="name" var="secName"/>
             
             <div id="content" >
             	<form id="emp" action="modi" method="post">
				...
				<thead>
					...
					<tr>
						<td>${desc.num}</td>
						<td>${desc.title}</td>
						<td>${desc.reporttype}</td>
						<td>${desc.author}</td>
						<td>${desc.regdate}</td>
						<td>${desc.reportdate}</td>
						<td>${desc.reporter}</td>
						<td>${desc.reportcontents}</td>
					</tr>
				</tbody>
			</table>
			</form>
			...
			<br>
	<a href="list"><button class="btn btn-primary" type="button">목록보기</button></a>
	<c:choose>
	<c:when test="${desc.reporttype=='홍보'}">
	<button type="button" class="btn btn-primary" onclick="delPromotion(${desc.reportnum});">삭제</button>
	</c:when>
	<c:when test="${desc.reporttype=='비디오'}">
	<button type="button" class="btn btn-primary" onclick="delVideo(${desc.reportnum});">삭제</button>
	</c:when>
	<c:when test="${desc.reporttype=='오디오'}">
	<button type="button" class="btn btn-primary" onclick="delAudio(${desc.reportnum});">삭제</button>
	</c:when>
	</c:choose>
	<button type="button" onclick="complete(${desc.num});" class="btn btn-primary">처리하기</button>
	<p>
	<c:choose>
		<c:when test="${desc.reporttype=='홍보'}">
		    <a href="<c:url value='/promotion/desc?num=${desc.reportnum}'/>">게시글 이동</a> 
		</c:when>
		<c:when test="${desc.reporttype=='비디오'}">
			<a href="<c:url value='/video/listReport?num=${desc.reportnum}'/>">게시글 이동</a>
		</c:when>
		<c:when test="${desc.reporttype=='오디오'}">
			<a href="<c:url value='/recodes/listReport?num=${desc.reportnum}'/>">게시글 이동</a> 
		</c:when>
	</c:choose>
