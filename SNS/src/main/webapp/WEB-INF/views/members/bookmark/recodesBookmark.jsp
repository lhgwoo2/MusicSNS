...
<script type="text/javascript">
  ...
  $(function(){
  	$('#page-selection').bootpag({
          total:  Math.floor(${list[0].totalRows-1}/6+1),			// ${totalPages}
          page: ${page},			// ${curPage}
          maxVisible: 4,		// rpp,  서버로 전달이 되어야 한다. totalPages를 구하는데 사용
          leaps: true,
          firstLastUse: true,
          first: '←',
          last: '→',
        ...
        //location.href="list?page="+num+"&rpp=4"
      	var dataObj={};
      	dataObj.page=num;
      	dataObj.type=type;
      	dataObj.key=key;
  ...
  //댓글창에서 enter눌러을때
  		            ...
  		            success: function(res) {
  		            		if(refComment==0){
  			              		    if(listsizeComment==0){
  			              		    	$("#postsComment").html(res);
  			              		    }else{
  			              		    	$("#postsComment").prepend(res);
  			              		    }
  							 } else{
  	...
  	function newspeedComment(num){
  		...
  		//더보기 버튼을 위한 사이즈를 가져온다.
  		    $.ajax({
  		        url: "<c:url value='/recodes/getcommentsize'/>",
  		        data:serialData,
  		        type:'post', //get , post 둘중하나
  		        dataType: 'json',
  		        success: function(res) {
  		  ...
  		  //첫번쨰 댓글 가져오기
  		  $.ajax({
  		       url: "<c:url value='/recodes/newspeedComment'/>",
  		       data:serialData,
  		       type:'get', //get , post 둘중하나
  		       dataType: 'html',
  		       success: function(res) {
  		    	$('#postsComment').html(res);				//댓글집어넣기
  		    	initcommentText = $('#postsComment').html();		 //초기 댓글 셋팅하기
  		    	$('#answerModal').modal();
  		       }, 
  		    error:function(xhr, status, error){
  			alert(error);
  	...
  	function moreComment(){
  	    ...
  	    // 데이터를 가져온다.
  	    $.ajax({
  	        url: "<c:url value='/recodes/getcomment'/>",
  	        data:serialData,
  	        type:'post', //get , post 둘중하나
  	        dataType: 'html',
  	        success: function(res) {
  	      	  	 $('#postsComment').append(res);
  	               $('#loading').hide();
...
<body>
<div id="listpage" style="background-image: url('/SNS/resources/background/galaxy.jpg');">
	...
	<!--navbar 오른쪽 부분 -->
			<div class="collapse navbar-collapse" id="example-navbar-primary">
				<ul class="nav navbar-nav navbar-left">
			        ...
			        <li class="active"><a href="<c:url value='/mypage/myInfo?id=${secName}'/>">MyPage</a></li>
...
<div class="container-fluid text-center">
  <div class="row content">
	      ...
	      <p><a href="bookmarkVideo">북마크 비디오</a></p>
	 	  <p><a href="bookmarkRecodes">북마크 오디오</a></p>
	 	  <p><a href="bookmarkPromotion">북마크 홍보</a></p>
	 	  <p><a href="myLoadVideo?num=${user.num}">내가 올린 비디오</a></p>
	 	  <p><a href="myLoadAudio?num=${user.num}">내가 올린 오디오</a></p>
	 	  <p><a href="myLoadPromotion?num=${user.num}">내가 올린 홍보</a></p>
	      <p><a href="<c:url value='/mypage/myInfo?id=${secName}'/>">내정보보기</a></p>
	      <p><a href="updateForm?num=${user.num}">회원정보수정</a></p>
	      <p><a href="pwdUpdate?num=${user.num}">비밀번호변경</a></p>
	      <p><a href="deletAccount?num=${user.num}">회원탈퇴</a></p>
	   </div>
		<div class="col-sm-2 sidenav" style="background: #FFFFFF; background : rgba(0, 0, 0, 0.0);">
      	  	<div id="main2" >  
			  <span id="index" style="font-size:30px;cursor:pointer" onclick="openNav2()">&#9776;INDEX</span>
			</div>  
		</div>
     ...
     <!-- 게시글 쓰기 영역 -->
 	 ...
 	 <br>
      	...
      	<c:forEach var="recode" items="${list}" begin="0" end="2" varStatus="state">
    		...
    		<div id="tablee" class="thumbnail" style="height: 100%; width: 100%">
			...
			<span class="caption" style="float: right;">
				<sec:authorize access="${secName==recode.author}">
					<a href="<c:url value='/recodes/updateForm?num=${recode.num}'/>">
						<button type="button" class="btn btn-default" style="width: 70px; height: 40px;">수정 <img src="/SNS/resources/markImage/edit.png" style="width: 18px; height: 18px; "></button>
					</a>
						<button type="button" onclick="del(${recode.num},'${recode.filename}');" class="btn btn-default" style="width: 70px; height: 40px; ">삭제 <img src="/SNS/resources/markImage/trash.png" style="width: 18px; height: 18px;"></button>
					</sec:authorize>
			</span>
			...
			<div class="row">
      	<c:forEach var="recode" items="${list}" begin="3" end="5" varStatus="state">
			 <div class="col-md-4">
      		
      			    		
    		<div id="tablee" class="thumbnail" style="height: 300px; width: 100%">
	...
	<div id="page-selection" align="center"></div>
 	...
 	<form id="searchForm" method="get" action="bookmarkRecodes">
 	<span class="col-xs-2">
    ...
    <span class="col-xs-8" >
		 <input type="text" name="key" class="form-control"  placeholder="검색어 입력" style="color: white;">
	</span> 
		<span class="col-xs-2">
		<button type="submit" id="btn" class="btn btn-primary">검색</button> 
		</span>
 	</form>
 	</div>
	...
	<div class="col-sm-2 sidenav" style="background: #FFFFFF; background : rgba(0, 0, 0, 0.0); "> 
		<div id="mySidenav" class="sidenav1">
		  <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
			<c:forEach var="bookmark" items="${list2}" begin="0" end="4" varStatus="state">
				<div class="well" style="margin-bottom: 20px; padding-bottom: 30px; word-break:break-all; overflow: auto;">
				<p style="float: left">공연제목 : ${bookmark.title}</p><br>
				<p style="float: left">공연시작일 : <fmt:formatDate type="both" dateStyle="short" timeStyle="short" value="${bookmark.openingday}" /></p><br>
				<p style="float: left; text-align: left;">공연장소 : ${bookmark.region}</p><br>
				</div>
			</c:forEach>
		</div>
		
		<div id="main" >  
		  <span style="font-size:30px;cursor:pointer" onclick="openNav()">&#9776;Notice</span>
		</div> 
	</div>
