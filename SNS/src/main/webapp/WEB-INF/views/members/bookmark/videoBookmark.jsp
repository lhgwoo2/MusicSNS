...
<script type="text/javascript">
  ...
  $(function(){
  	$('#page-selection').bootpag({
          total:  Math.floor(${list[0].totalRows-1}/3+1),			// ${totalPages}
          page: ${page},			// ${curPage}
  ...
  //댓글창에서 enter눌러을때
  	function eventEnter(evt){
  		      if(evt.keyCode==13){
  		        ...
  		        $.ajax({
  		            url: "<c:url value='/video/setcomment'/>",
  		            data:msg,
  	...
  	function newspeedComment(num){
  		...
  		//더보기 버튼을 위한 사이즈를 가져온다.
  		    $.ajax({
  		        url: "<c:url value='/video/getcommentsize'/>",
  		        data:serialData,
  		        type:'post', //get , post 둘중하나
  		        dataType: 'json',
  		        success: function(res) {
  		  ...
  		  //첫번쨰 댓글 가져오기
  		  $.ajax({
  		       url: "<c:url value='/video/newspeedComment'/>",
  		       data:serialData,
  	...
  	function moreComment(){
  	    ...
  	    // 데이터를 가져온다.
  	    $.ajax({
  	        url: "<c:url value='/video/getcomment'/>",
  	        data:serialData,
  	        type:'post', //get , post 둘중하나
  	        dataType: 'html',
  	        success: function(res) {
  ...
  <div class="row content">
    ...
    <div class="col-sm-8 text-left">
     ...
     <!-- 게시글 쓰기 영역 -->
 	 ...
 	 <p style="color: white;">비디오 북마크</p>
 	
 	 <hr>
 	 <br>
      	<div class="row">
      	<c:forEach var="video" items="${list}" begin="0" end="2" varStatus="state">
			 <div class="col-md-4">
			...
			<br>
			<video width="100%" controls  loop preload="none">
  						<source src="/SNS/resources/video/${video.filename}" type='video/mp4' ></video>
  			</div>
  						
			<hr>
			<span class="caption" style="float: right;">
					<sec:authorize access="${secName==video.author}">
			               	<a href="<c:url value='/video/videoupdate?num=${video.num}'/>">
			               ...
			               <button type="button"  onclick="del(${video.num},'${video.filename}');"class="btn btn-primary" style="width: 80px; height: 40px;">삭제 <img src="/SNS/resources/markImage/trash.png" style="width: 18px; height: 18px;"></button>
					</sec:authorize>
			</span>
			
			<span>
			<!-- 댓글 시작-->
					<span style="float: left;"><img src="/SNS/resources/markImage/reply.png" style="width: 20px; height: 20px;"><a style="cursor: pointer;" data-toggle="modal" onclick="newspeedComment(${video.num})" >댓글 보기&쓰기</a></span>
	...
	<div id="page-selection" align="center"></div>
 	...
 	<form id="searchForm" method="get" action="bookmarkVideo">
 	<span class="col-xs-2">
